...
parser.add_argument(
    "-x", "--upload", dest="upload", help="Enable Uploading", action="store_true"
)
...
class PhotoBooth:
    def __init__(self, zen_user=None, zen_pass=None, upload=None):
        ...
        self._startup()
        self._init_zenfolio()
        self.attract = self.rpi.run_as_thread(
            self.panel.scroll,
            start=True,
            speed=0.01,
    ...
    def _init_zenfolio(self):
        ...
        if self.net_www and self._zen_user and self._zen_pass:
            init_scroll = self.rpi.run_as_thread(
                self.panel.scroll,
                start=True,
                speed=0.01,
            ...
            self.zen_api.profile = self.zen_api.LoadPublicProfile(
                username=zen_profile_name
            )
            root_group_id = self.zen_api.profile.RootGroup.Id
            self.zen_api.root_grp = self.zen_api.LoadGroupHierarchy(
                username=zen_profile_name
            )
            ...
            if not found:
                ...
                self.zen_api.UpdateGroupAccess(
                    group_id=event_grp_id, group_access=access
                )

            self.zen_api.event_grp = self.zen_api.LoadGroup(
                id=event_grp_id, recursive=True
            )
            ...
            if not found:
                ...
                self.zen_api.UpdateGroupAccess(
                    group_id=booth_grp_id, group_access=access
                )

            self.zen_api.booth_grp = self.zen_api.LoadGroup(
                id=booth_grp_id, recursive=True
            )
            ...
            if not found:
                ...
                gallery = {
                    "Title": f"{self.today}",
                    "CustomReference": f"events/{self.today}",
                }
                result = self.zen_api.CreatePhotoSet(
                    group_id=booth_grp_id, photoset=gallery
                )
                ...
                self.zen_api.UpdatePhotoSetAccess(
                    photoset_id=booth_gallery_id, photoset_access=access
                )

            self.zen_api.booth_gallery = self.zen_api.LoadPhotoSet(
                set_id=booth_gallery_id, with_photos=False
            )
    ...
    def _startup(self):
        ...
        panel_test = self.rpi.run_as_thread(
            self.panel.panel_test, executions=1, start=True
        )

        # self.camera = self.rpi.add_camera(name="main", model="Canon EOS 1100D")
        self.camera = self.rpi.add_camera(name="main", model="Canon EOS 800D")
    ...
    def print_receipt(self, shot_name: str = None):
        ...
        if not shot:
            print(
                f"Attempting to print {shot_name} but it doesn't exist in 'self.shot_data'"
            )
            return False

        if shot["print_n"] >= max_prints:
            print(
                f"Max prints ({max_prints}) exceeded for "
        ...
        self.rpi.run_as_thread(
            self.panel.flash, text="Printing...   ", executions=1, start=True
        )
        ...
        self.printer.ln()
        self.printer.text(
            text="Mention this photobooth when you book your next "
        ...
        if shot["uploaded"]:
            ...
            self.printer.ln()
            self.printer.text(text="Please send us an email at")
            self.printer.ln()
            self.printer.text(text="contact@capturingtimephoto.net")
            self.printer.ln()
    ...
    def capture(self, camera: object = None, upload: bool = True):
        ...
        _capture = self.rpi.run_as_thread(
            target=camera.capture, executions=1, start=True
        )
        ...
        awesome = self.rpi.run_as_thread(
            self.panel.flash, text="AWESOME!", executions=1, start=True
        )  # noqa: E501
...
if __name__ == "__main__":
    booth = PhotoBooth(
        zen_user=getattr(args, "username", None),
